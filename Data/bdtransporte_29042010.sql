...
CREATE TABLE `tdetalle_anticipo` (
  ...
  `nmonto` double NOT NULL,
  KEY `FK_tdet_ant_tanticipo` (`nid_anticipo`),
  KEY `FK_tdet_ant_ttipo_gasto` (`cid_tipogasto`),
  CONSTRAINT `FK_tdet_ant_tanticipo` FOREIGN KEY (`nid_anticipo`) REFERENCES `tanticipo` (`nid_anticipo`),
  CONSTRAINT `FK_tdet_ant_ttipo_gasto` FOREIGN KEY (`cid_tipogasto`) REFERENCES `ttipo_gasto` (`cid_tipogasto`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `truta` (
  ...
  KEY `FK_truta_torigdest_d` (`nid_destino`),
  KEY `FK_truta_torigdest_o` (`nid_origen`),
  CONSTRAINT `FK_truta_torigdest_d` FOREIGN KEY (`nid_destino`) REFERENCES `torigen_destino` (`nid_origdest`),
  CONSTRAINT `FK_truta_torigdest_o` FOREIGN KEY (`nid_origen`) REFERENCES `torigen_destino` (`nid_origdest`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
