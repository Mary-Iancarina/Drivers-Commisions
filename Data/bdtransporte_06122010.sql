...
CREATE DEFINER=`root`@`localhost` FUNCTION `Fn_Corr_Nid_Anticipo`(tNid_Sucursal int(4)) RETURNS int(11)
BEGIN
   ...
   Set lnReturn = (select nid_anticipo from tcorrelativo where nid_sucursal = tNid_Sucursal);
   update tcorrelativo set nid_sucursal = lnReturn where nid_sucursal = tNid_Sucursal;

   Return lnReturn;


/*
CREATE DEFINER=`root`@`%` FUNCTION `FN_TipoCliente`(tnValor Integer) RETURNS varchar(200) CHARSET latin1
BEGIN
  Declare lcReturn  VarChar(200);
  Declare lnP,lnTC Integer;
  Create Temporary Table IF NOT EXISTS tTemp(nPar Integer,nTipoC Integer);
  Insert Into tTemp(nPar,nTipoC) Select nId_Parametro,nId_TipoCli From tdetalle_par_cob Where nId_Parametro = tnValor;

  Set lcReturn ='';
  Set lnP=0;
  Set lnTC=0;

  While Exists(Select * From tTemp) Do
      Set lnP  = (Select nPar From tTemp Order by nTipoC Limit 1);
      Set lnTC = (Select nTipoC From tTemp Order by nTipoC Limit 1);
      Set lcReturn = Concat(lcReturn ,IF(lcReturn<>'','-',''),(Select Cast(nTipoC As Char) From tTemp Where nPar = lnP And nTipoC = lnTC));
      Delete From tTemp Where nPar = lnP And nTipoC = lnTC;
  End While;
  Return lcReturn;
END
*/
...
/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Sp_Reemplazar_Anticipo`(tnid_anticipo Integer,tnid_sucursal int(4),tcid_vehiculo varchar(10),tnid_sucursal_v int(4),tcid_chofer varchar(9),tnid_sucursal_c int(4),tcobservacion text,tcid_usuario varchar(20))
BEGIN
   ...
   Start Transaction;
      ...
      Create Temporary Table ttAnticip_tViaje (nid_anticipo Integer,index using btree (nid_anticipo),nid_viaje Integer,index using btree (nid_viaje));
