...
CREATE TABLE `tcomision_choferes` (
  ...
  `nporc_com` int(2) unsigned NOT NULL COMMENT '% de comisión de la ruta.',
  `ntabulador` double NOT NULL COMMENT 'tabulador de viaje para el tipo de vehículo en la ruta.',
...
INSERT INTO `tperiodo` (`cid_periodo`,`dfecha_desde`,`dfecha_hasta`,`cdescripcion`,`cid_usuario`,`dfecha_act`,`nstatus`) VALUES 
 ...
 ('16101511','2010-10-16','2010-11-15','OCT 2010 - NOV 2010','P','2010-10-18 18:11:28',1);
...
CREATE TABLE `tsucursal` (
  ...
  `nporc_com` int(3) unsigned default NULL COMMENT 'Porcentaje a partir del cual se paga comisiones al jefe de almacen y montacarguista de la sucursal.',
...
Cancel:BEGIN
   ...
   Start Transaction;
                                                /*
      if not exists(select * from tconfig where  = 1) then
         ROLLBACK;
         LEAVE Cancel;
      end if;
                                                  */
      ...
      if not exists(select count(cid_periodo) from tperiodo where nstatus = 1 having count(cid_periodo) > 1) then
         ROLLBACK;
         LEAVE Cancel;
      end if;

      select nid_viaje,sum(nmonto/count_anticipo) as nmonto from vw_viajes
         where ((nstatus_r = 1 and (dfecha_r between '2010-09-16' and '2010-10-31'))) AND nid_sucursal = 1
         group by nid_viaje
