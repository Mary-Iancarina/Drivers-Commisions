...
INSERT INTO `tchofer` (`cid_chofer`,`cnombre`,`ccontraparte`,`cficha_nom`,`nid_sucursal`,`cid_usuario`,`dfecha_act`,`ncontratado`,`nactivo`) VALUES 
 ...
 ('8603178','MENDEZ R. EDDYS E .',' ',' ',1,'P','2010-10-18 14:55:30',1,1),
...
INSERT INTO `tperiodo` (`cid_periodo`,`dfecha_desde`,`dfecha_hasta`,`cdescripcion`,`cid_usuario`,`dfecha_act`,`nstatus`) VALUES 
 ('16091510','2010-09-16','2010-11-15','SEPT 2010 - OCT 2010','P','2010-10-18 17:56:34',1),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Sp_Calc_Com_Chof`()
Cancel: BEGIN
   Declare nError Int(10);
   Declare ldfecha_desde,ldfecha_hasta date;
   Declare lcid_periodo VARCHAR(10);
   DECLARE finished INTEGER DEFAULT 0;
      DECLARE ttperiodo CURSOR FOR SELECT cid_periodo,dfecha_desde,dfecha_hasta FROM tperiodo where nstatus=1;
      OPEN ttperiodo;
      FETCH ttperiodo INTO lcid_periodo,ldfecha_desde,ldfecha_hasta;
      close ttperiodo;
 --     select ldfecha_desde,ldfecha_hasta;
      ...
      if not exists(select cid_periodo,dfecha_desde,dfecha_hasta from tperiodo where nstatus = 1) then
         ROLLBACK;
         LEAVE Cancel;
      end if;

      if exists(select count(cid_periodo) from tperiodo where nstatus = 1 having count(cid_periodo) > 1) then
         ROLLBACK;
         LEAVE Cancel;
      end if;

      -- 1:Liviano (<13Ton),  2:Mediano (13Ton<=X<=25Ton),  3:Pesado(>25Ton)
