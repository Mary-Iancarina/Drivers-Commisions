...
INSERT INTO `torigen_destino` (`nid_origdest`,`cdescripcion`,`cid_ciudad`,`cid_usuario`,`dfecha_act`,`nactivo`,`norigen`,`nsucursal`) VALUES 
 ...
 (5,'SUCURSAL MARACAIBO','2301','P','2010-06-14 09:10:09',0,0,0),
...
INSERT INTO `tvehiculo` (`cid_vehiculo`,`cdescripcion`,`cmarca`,`cserial_motor`,`cserial_carroc`,`ntara`,`ncapacidad`,`ctipo_vehic`,`ncosto`,`cid_usuario`,`dfecha_act`,`npropio`,`nvolumen`,`nactivo`,`nid_sucursal`) VALUES 
 ('V1','V1','M1','SM1','SC1',1000,1000,'\"LIVIANO\"',0,'P','2010-07-14 15:32:11',1,0,1,1),
 ('V2','V2','M2','SM2','SC2',2000,2999,'MEDIANO\"',0,'P','2010-07-14 15:32:40',1,0,1,1),
 ('V3','V3','M3','SM3','SC3',3000,3000,'\"PESADO\"',0,'P','2010-07-14 15:34:16',1,0,1,2),
 ('V4','V4','M4','SM4','SC4',4000,4000,'MEDIANO\"',0,'P','2010-07-14 15:35:14',0,0,1,1),
 ('V5','V5','M5','SM5','SC5',5000,5000,'\"PESADO\"',0,'P','2010-07-14 15:36:08',1,0,1,3),
 ('V6','V6','M6','SM6','SC6',6000,6000,'MEDIANO\"',0,'P','2010-07-14 15:36:35',0,0,1,4);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_tviaje_tanticipo`(cid_chofer varchar(9),cid_vehiculo varchar(10),cguia varchar(10),corden_carga varchar(10),dfecha_guia date,npeso_neto double,cid_usuario varchar(20),nid_origen int(4),nid_destino int(4),cid_periodo varchar(10),ntipo_viaje tinyint(1),nid_sucursal_guia int(4),cnro_recibo varchar(10),nmonto double,cobservacion text)
BEGIN
  /*      Start Transaction;
...
/*
CREATE DEFINER=`root`@`%` PROCEDURE `SP_CalcularComVentas`(tnValor Integer)
BEGIN
  ...
  Declare lnComi,lnComiVenta Int(10);
  Declare lcGrupo Varchar(4);
  Declare lcBono Varchar(2);
  ...
  Create Temporary Table ttCuotasBonos(nId Int(10) UNSIGNED NOT NULL Primary Key DEFAULT NULL AUTO_INCREMENT,nId_Comisionista Int(5),cId_Grupo Varchar(4),nCuota Int(10),nPor_Bono Double(15,2),nMonto_Bono Double(15,2),cId_Sucursal VarChar(2),nPagxCuota Int(3),cId_Bono Varchar(2));
  Insert Into ttCuotasBonos (nId_Comisionista,cId_Grupo,nCuota,nPor_Bono,nMonto_Bono,cId_Sucursal,nPagxCuota,cId_Bono)Select nId_Comisionista,cId_Grupo,nCuota,nPor_Bono,nMontoBono,lcSucursal,nPagxCuota,tDetalle_ConfigCalculos.cId_Bono From tDetalle_ConfigCalculos Inner Join tBonos On tDetalle_ConfigCalculos.cId_Bono=tBonos.cId_Bono Where nActivacion = 0 And nId_ConfigCal = tnValor And nId_Comisionista Not In (Select Distinct nId_Comisionista From tDetalle_config_Comi)  Order By 1,Cast(cId_Grupo As Decimal);
  ...
  While Exists(Select * From ttCuotasBonos) Do
      ...
      Set lcBono      = (Select cId_Bono From ttCuotasBonos Where nId = lnId);
      Set lnBono      = (Select TDCB.nMontoBono From tDetalle_Comi_Bono TDCB Inner Join tDetalles_Bonos_Grupos TDBG On TDCB.cId_Bono = TDBG.cId_Bono Inner Join tComisionistas TC ON TC.cId_TipoC = TDCB.cId_TipoC Where nId_Comisionista = lnComi And TDBG.cId_Grupo = lcGrupo And TDBG.cId_Bono = lcBono);
      Set lnPagxCuota = (Select nPagxCuota From ttCuotasBonos Where nId = lnId);
      Set lnComiVenta = (Select nVentasNac From tComisionistas Where nId_Comisionista = lnComi And cId_Sucursal = lcSucursal Limit 1);

      Delete From ttVentasDevSuma;
      Insert Into ttVentasDevSuma (cId_Producto,nTotal) Select TP.cId_Producto,If(TP.cFactor='*',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*TP.nValor,If(TP.cFactor='+',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))+TP.nValor,If(TP.cFactor='-',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))-TP.nValor,If(TP.cFactor='/',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))/TP.nValor,0)))) As Total From tDocumentos TD Inner Join tDetalle_Documentos TDD On TD.nId_Numero = TDD.nId_Numero And TD.nTipo = TDD.nTipo And TD.cId_Sucursal = TDD.cId_Sucursal Inner Join tDetalles_Grupos_Prod TDGP On TDGP.cId_Producto = TDD.cId_Producto Inner Join tProductos TP On TP.cId_Producto = TDD.cId_Producto Inner Join tdetalles_bonos_grupos TDBG On TDGP.cId_Grupo =TDBG.cId_Grupo Where TDBG.cId_bono = lcBono and TD.nId_Comisionista = lnComi And TDGP.cId_Grupo = lcGrupo And (TD.dFecha >=ldFechaD And TD.dFecha<=ldFechaH) And TD.nTipo In (10,11,12,13) And ((lnComiVenta=0 And TD.cId_Sucursal = lcSucursal) Or (lnComiVenta=1)) Group By 1;
      Set lnVenta     = (Select IF(IsNull(Sum(nTotal))=1,0,Sum(nTotal)) From ttVentasDevSuma);

      Delete From ttVentasDevSuma;
      Insert Into ttVentasDevSuma (cId_Producto,nTotal) Select TP.cId_Producto,If(TP.cFactor='*',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*TP.nValor,If(TP.cFactor='+',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))+TP.nValor,If(TP.cFactor='-',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))-TP.nValor,If(TP.cFactor='/',if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))/TP.nValor,0)))) As Total From tDocumentos TD Inner Join tDetalle_Documentos TDD On TD.nId_Numero = TDD.nId_Numero And TD.nTipo = TDD.nTipo And TD.cId_Sucursal = TDD.cId_Sucursal Inner Join tDetalles_Grupos_Prod TDGP On TDGP.cId_Producto = TDD.cId_Producto Inner Join tProductos TP On TP.cId_Producto = TDD.cId_Producto  Inner Join tdetalles_bonos_grupos TDBG On TDGP.cId_Grupo =TDBG.cId_Grupo Where TDBG.cId_bono = lcBono and  TD.nId_Comisionista = lnComi And TDGP.cId_Grupo = lcGrupo And (TD.dFecha >=ldFechaD And TD.dFecha<=ldFechaH) And TD.nTipo In (14,15,16,17) And ((lnComiVenta=0 And TD.cId_Sucursal = lcSucursal) Or (lnComiVenta=1)) Group By 1;
      Set lnDev       = (Select IF(IsNull(Sum(nTotal))=1,0,Sum(nTotal)) From ttVentasDevSuma);

      Delete From ttVentasDevResta;
      Insert Into ttVentasDevResta (nId_Cliente,cId_Producto,nTotal) Select TD.nId_Cliente,TP.cId_Producto,If(TP.cFactor='*',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))*TP.nValor,If(TP.cFactor='+',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))+TP.nValor,If(TP.cFactor='-',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))-TP.nValor,If(TP.cFactor='/',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))/TP.nValor,0)))) As Total From tDocumentos TD Inner Join tDetalle_Documentos TDD On TD.nId_Numero = TDD.nId_Numero And TD.nTipo = TDD.nTipo And TD.cId_Sucursal = TDD.cId_Sucursal Inner Join tDetalle_Config_Comi TDC On TDC.nId_Cliente=TD.nId_Cliente Inner Join tDetalles_Grupos_Prod TDGP On TDGP.cId_Producto = TDD.cId_Producto Inner Join tProductos TP On TP.cId_Producto = TDD.cId_Producto  Inner Join tdetalles_bonos_grupos TDBG On TDGP.cId_Grupo =TDBG.cId_Grupo Where TDBG.cId_bono = lcBono and  TD.nId_Comisionista = lnComi And TDGP.cId_Grupo = lcGrupo And TDC.nTipoPar = 1 And TDC.nRestaVen=1 And (TD.dFecha >=ldFechaD And TD.dFecha<=ldFechaH) And TD.nTipo In (10,11,12,13) And ((lnComiVenta=0 And TD.cId_Sucursal = lcSucursal) Or (lnComiVenta=1)) Group By 1,2;
      Set lnVentaCli  = (Select IF(IsNull(Sum(nTotal))=1,0,Sum(nTotal)) From ttVentasDevResta);

      Delete From ttVentasDevResta;
      Insert Into ttVentasDevResta (nId_Cliente,cId_Producto,nTotal) Select TD.nId_Cliente,TP.cId_Producto,If(TP.cFactor='*',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))*TP.nValor,If(TP.cFactor='+',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))+TP.nValor,If(TP.cFactor='-',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))-TP.nValor,If(TP.cFactor='/',(if(IsNull(Sum(TDD.nCantidad))=1,0,Sum(TDD.nCantidad))*(TDC.nPorcVenta/100))/TP.nValor,0)))) As Total From tDocumentos TD Inner Join tDetalle_Documentos TDD On TD.nId_Numero = TDD.nId_Numero And TD.nTipo = TDD.nTipo And TD.cId_Sucursal = TDD.cId_Sucursal Inner Join tDetalle_Config_Comi TDC On TDC.nId_Cliente=TD.nId_Cliente Inner Join tDetalles_Grupos_Prod TDGP On TDGP.cId_Producto = TDD.cId_Producto Inner Join tProductos TP On TP.cId_Producto = TDD.cId_Producto  Inner Join tdetalles_bonos_grupos TDBG On TDGP.cId_Grupo =TDBG.cId_Grupo Where TDBG.cId_bono = lcBono and  TD.nId_Comisionista = lnComi And TDGP.cId_Grupo = lcGrupo And TDC.nTipoPar = 1 And TDC.nRestaVen=1 And (TD.dFecha >=ldFechaD And TD.dFecha<=ldFechaH) And TD.nTipo In (14,15,16,17) And ((lnComiVenta=0 And TD.cId_Sucursal = lcSucursal) Or (lnComiVenta=1)) Group By 1,2;
      ...
      If lnCumplimiento < 100  Then
        Set lnCumplimiento = Round(lnCumplimiento,2);
      ...
      Insert Into tComisiones_Ventas(nId_ConfigCal,nId_Comisionista,cId_Grupo,nVenta,nDevoluciones,nOtrasAsignaciones,nOtrasDeducciones,nBono_Asig,nPorcBono,cId_Bono,nBonoTotal)
      Values (tnValor,lnComi,lcGrupo,lnVenta,lnDev,0,lnDesc,Round(lnBonoAsig,2),Round(lnPor_BonoAsig),lcBono,lnBono);
